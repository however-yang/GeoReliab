...
gpu_ledger=$8
final_evidence=$9
attempt_root="$runtime_root/authorization-attempts/attempt-03"
...
python -m georeliab_mve v4-attempt03-revalidate-resources \
  --worktree "$worktree" \
...
python -m georeliab_mve v4-attempt03-gpu-preflight \
  --worktree "$worktree" \
  --resource-receipt "$resource_receipt" \
  --output "$hardware_snapshot"

python -m georeliab_mve v4-attempt03-create-execution-authorization \
  --worktree "$worktree" \
  --runtime-root "$runtime_root" \
  --resource-receipt "$resource_receipt" \
...
python -m georeliab_mve \
  v4-attempt03-validate-execution-authorization "$authorization"
