...
SCHEMA_PREFIX = 'georeliab-v4-attempt-04'
...
DECISION_SCHEMA = f'{SCHEMA_PREFIX}-gpu-selection-decision-1.0'

BASE_COMMIT = '6e91a418983162d50a78a008fb41c540b8edf4c0'
BASE_TREE = '2f9b2641136ad864e3864eb92ad67ffc18e54056'
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise V4ExecutionError('V4_ATTEMPT04_JSON_UNREADABLE') from exc
    if not isinstance(value, dict):
        raise V4ExecutionError('V4_ATTEMPT04_JSON_OBJECT_REQUIRED')
    return value
...
def _validate_signature(
    ...
    if (
        payload.get('base_commit') != BASE_COMMIT
        or payload.get('base_tree') != BASE_TREE
    ):
        raise V4ExecutionError('V4_ATTEMPT04_BASE_REVISION_MISMATCH')
...
def _atomic_json(
    ...
    if path.exists() or partial.exists():
        raise V4ExecutionError('V4_ATTEMPT04_ARTIFACT_COLLISION')
    try:
...
def _git_value(worktree: Path, *args: str) -> str:
    try:
        ...
        raise V4ExecutionError(
            'V4_ATTEMPT04_AUTHORIZATION_REVISION_UNRESOLVED'
        ) from exc
...
def _tooling_revision(worktree: Path) -> tuple[str, str]:
    ...
    if _git_value(worktree, 'status', '--porcelain'):
        raise V4ExecutionError('V4_ATTEMPT04_WORKTREE_NOT_CLEAN')
    return commit, tree
...
def _attempt_path(path: Path) -> Path:
    ...
    if any(
        ...
        raise V4ExecutionError('V4_ATTEMPT04_HISTORY_REUSE_FORBIDDEN')
    if parts.count(ATTEMPT_ID) != 1:
        raise V4ExecutionError('V4_ATTEMPT04_PATH_SCOPE_MISMATCH')
    return resolved
...
_ATTEMPT_ARTIFACT_NAMES = {
    'resource': 'v4-resource-revalidation.json',
    'snapshot': 'v4-hardware-snapshot.json',
    'decision': 'v4-gpu-selection-decision.json',
    'receipt': 'v4-gpu-selection-receipt.json',
    'authorization': 'v4-execution-authorization.json',
}
_ALLOWED_PHASE_PREDECESSORS = {
    'resource': frozenset(),
    'preflight': frozenset({'resource'}),
    'authorization': frozenset({'resource', 'snapshot'}),
}


def _artifact_paths(attempt_root: Path) -> dict[str, Path]:
    return {
        label: attempt_root / name
        for label, name in _ATTEMPT_ARTIFACT_NAMES.items()
    }


def _guard_attempt_phase(attempt_root: Path, phase: str) -> None:
    allowed = _ALLOWED_PHASE_PREDECESSORS.get(phase)
    if allowed is None:
        raise V4ExecutionError('V4_ATTEMPT04_PHASE_UNKNOWN')
    paths = _artifact_paths(attempt_root)
    for path in paths.values():
        if path.with_name(path.name + '.partial').exists():
            raise V4ExecutionError('V4_ATTEMPT04_ARTIFACT_COLLISION')
    present = {label for label, path in paths.items() if path.exists()}
...
def _validate_decision_payload(payload: Mapping[str, Any]) -> None:
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_DECISION_TAMPER')
    status = payload.get('status')
    reason = payload.get('reason_code')
    receipt_generated = payload.get('pass_receipt_generated')
    authorization_generated = payload.get(
    ...
    if status == 'BLOCKED':
        if (
            reason not in {
                ...
                'V4_NO_ELIGIBLE_IDLE_GPU',
            }
            or receipt_generated is not False
            or authorization_generated is not False
        ):
            raise V4ExecutionError('V4_ATTEMPT04_DECISION_TAMPER')
        if (
            payload.get('gpu_receipt_path') is not None
            or payload.get('gpu_receipt_sha256') is not None
            or payload.get('execution_authorization_path') is not None
            or payload.get('execution_authorization_sha256') is not None
        ):
            raise V4ExecutionError('V4_ATTEMPT04_DECISION_TAMPER')
    elif status == 'PASS':
        if (
            reason != 'V4_MVE_EXECUTION_AUTHORIZED'
            or receipt_generated is not True
            or authorization_generated is not True
            or not payload.get('gpu_receipt_path')
            or not _is_sha(payload.get('gpu_receipt_sha256'))
            or not payload.get('execution_authorization_path')
            or not _is_sha(
                payload.get('execution_authorization_sha256')
            )
        ):
            raise V4ExecutionError('V4_ATTEMPT04_DECISION_TAMPER')
    else:
    ...
    return _signed(
        {
            ...
            'resource_receipt_path': str(resource_receipt_path.resolve()),
            'resource_receipt_sha256': _sha256_file(resource_receipt_path),
            'hardware_snapshot_path': (
                None if snapshot_path is None else str(snapshot_path.resolve())
            ),
            'hardware_snapshot_sha256': (
                None if snapshot_path is None else _sha256_file(snapshot_path)
            ),
            'gpu_receipt_path': (
                None if receipt_path is None else str(receipt_path.resolve())
            ),
            'gpu_receipt_sha256': receipt_sha256,
            ...
            'pass_receipt_generated': generated,
            'execution_authorization_generated': generated,
...
def _write_blocked_decision(
    *,
    ...
    decision_path = _artifact_paths(attempt_root)['decision']
    ...
    _atomic_json(decision_path, payload, _validate_decision_payload)
    return payload
...
def _file_binding(path: Path) -> dict[str, object]:
    if not path.is_file():
        raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_FILE_MISSING')
    return {
...
def _anchor_blob(worktree: Path, relative: str) -> bytes:
    try:
        ...
        raise V4ExecutionError(
            'V4_ATTEMPT04_SCIENTIFIC_ANCHOR_UNAVAILABLE'
        ) from exc
    return completed.stdout
...
def _validate_numerical_anchor(worktree: Path) -> list[dict[str, str]]:
    ...
    for relative in _NUMERICAL_ANCHOR_PATHS:
        ...
        if current != anchor:
            raise V4ExecutionError(
                'V4_ATTEMPT04_NUMERICAL_PATH_DRIFT'
            )
        rows.append(
            {
                'path': relative,
...
def _read_overlay(path: Path) -> dict[str, Any]:
    try:
        ...
        raise V4ExecutionError('V4_ATTEMPT04_OVERLAY_UNREADABLE') from exc
    if not isinstance(payload, dict):
        raise V4ExecutionError('V4_ATTEMPT04_OVERLAY_UNREADABLE')
    return payload
...
def _resource_bindings(
    *,
    ...
    if not isinstance(resources, Mapping):
        raise V4ExecutionError('V4_ATTEMPT04_OVERLAY_RESOURCES_REQUIRED')
    ...
    for path, key in (
        ...
        if not _is_sha(expected):
            raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_SHA_REQUIRED')
        _assert_file_digest(
            path, str(expected), 'V4_ATTEMPT04_RESOURCE_HASH_MISMATCH'
        )
    models = {
        'VGGT': {
    ...
    for model in models.values():
        ...
        if checkpoint['sha256'] != model['checkpoint_sha256']:
            raise V4ExecutionError(
                'V4_ATTEMPT04_MODEL_RESOURCE_HASH_MISMATCH'
            )
        ...
        if not isinstance(commits, Mapping) or any(
            ...
            raise V4ExecutionError(
                'V4_ATTEMPT04_UPSTREAM_COMMIT_REQUIRED'
            )
    ...
    if (
        ...
        raise V4ExecutionError(
            'V4_ATTEMPT04_RECTIFIED_SOURCE_IDENTITY_REQUIRED'
        )
    rectified = {
        'url': rectified_url,
...
def _validate_resource_payload(payload: Mapping[str, Any]) -> None:
    if payload.get('schema_version') != RESOURCE_SCHEMA:
        raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_SCHEMA_REQUIRED')
    if payload.get('attempt_id') != ATTEMPT_ID:
        raise V4ExecutionError('V4_ATTEMPT04_ID_MISMATCH')
    if (
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_RECEIPT_TAMPER')
    if payload.get('scientific_anchor_commit') != SCIENTIFIC_ANCHOR_COMMIT:
        raise V4ExecutionError('V4_ATTEMPT04_SCIENTIFIC_ANCHOR_MISMATCH')
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_GPU_PROBE_SCOPE_VIOLATION')
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_SCIENCE_LOCK_TAMPER')
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_NUMERICAL_PATH_DRIFT')
    ...
    for label, expected in _EXPECTED_FILE_DIGESTS.items():
        ...
        if (
            ...
            raise V4ExecutionError(
                'V4_ATTEMPT04_RESOURCE_RECEIPT_TAMPER'
            )
    _validate_signature(
        payload,
...
def _validate_resource_failure_payload(payload: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_BLOCKER_TAMPER')
    if (
        not _is_sha(payload.get('tooling_commit'), 40)
        or not _is_sha(payload.get('tooling_tree'), 40)
    ...
    _validate_signature(
        payload,
        'resource_revalidation_sha256',
        'V4_ATTEMPT04_RESOURCE_BLOCKER_TAMPER',
    )
...
def _revalidate_attempt04_resources_pass(
    *,
    worktree: Path,
    runtime_root: Path,
    rectified_root: Path,
...
def revalidate_attempt04_resources(
    *,
    worktree: Path,
    runtime_root: Path,
    rectified_root: Path,
    ...
    if output_path != canonical_output:
        raise V4ExecutionError('V4_ATTEMPT04_CANONICAL_OUTPUT_REQUIRED')
    _guard_attempt_phase(_required_attempt_root(runtime_root), 'resource')
    tooling_commit, tooling_tree = _tooling_revision(worktree)
    try:
        return _revalidate_attempt04_resources_pass(
            worktree=worktree,
            runtime_root=runtime_root,
        ...
        _write_blocked_decision(
            attempt_root=_required_attempt_root(runtime_root),
            reason_code='V4_RESOURCE_CLOSURE_REVALIDATION_FAILED',
            tooling_commit=tooling_commit,
            tooling_tree=tooling_tree,
            resource_receipt_path=output_path,
            snapshot_path=None,
        )
        return dict(payload)


def validate_attempt04_resources(path: Path) -> dict[str, Any]:
...
def _cuda_runtime(runner: Callable[[Sequence[str]], str]) -> str:
    ...
    if marker not in banner:
        raise V4ExecutionError('V4_ATTEMPT04_CUDA_RUNTIME_UNPROVEN')
...
def _process_text(pid: int, name: str) -> dict[str, str]:
    ...
    try:
        ...
        raise V4ExecutionError(
            'V4_ATTEMPT04_GPU_PROCESS_IDENTITY_UNPROVEN'
        ) from exc
...
def nvidia_smi_attempt04_inventory(
    *,
    ...
    try:
        rows = _csv_rows(
            ...
            14,
            'V4_ATTEMPT04_GPU_INVENTORY_UNAVAILABLE',
        )
    ...
    for row in _csv_rows(
        process_text,
        4,
        'V4_ATTEMPT04_GPU_PROCESS_ENUMERATION_UNPROVEN',
    ) if process_text.strip() else []:
        gpu_uuid, pid_raw, name, memory_raw = row
        pid = _required_int(
            pid_raw, 'V4_ATTEMPT04_GPU_PROCESS_IDENTITY_UNPROVEN'
        )
        ...
        processes.setdefault(gpu_uuid, []).append(
            {
                ...
                'used_memory_bytes': _required_int(
                    memory_raw,
                    'V4_ATTEMPT04_GPU_PROCESS_MEMORY_UNPROVEN',
                )
                * 1024
                * 1024,
                **identity,
    ...
    for row in rows:
        ecc = _required_int(
            row[12], 'V4_ATTEMPT04_GPU_ECC_HEALTH_UNPROVEN'
        )
        gpu_uuid = row[1]
        devices.append(
            {
                'index': _required_int(
                    row[0], 'V4_ATTEMPT04_GPU_INDEX_UNPROVEN'
                ),
                'uuid': gpu_uuid,
                'pci_bus_id': row[2],
                'model': row[3],
                'total_memory_bytes': _required_int(
                    row[4], 'V4_ATTEMPT04_GPU_MEMORY_UNPROVEN'
                )
                * 1024
                * 1024,
                'free_memory_bytes': _required_int(
                    row[5], 'V4_ATTEMPT04_GPU_MEMORY_UNPROVEN'
                )
                * 1024
                * 1024,
                'used_memory_bytes': _required_int(
                    row[6], 'V4_ATTEMPT04_GPU_MEMORY_UNPROVEN'
                )
                * 1024
                * 1024,
                'utilization_gpu_percent': _required_int(
                    row[7], 'V4_ATTEMPT04_GPU_UTILIZATION_UNPROVEN'
                ),
                'temperature_c': _required_int(
                    row[8], 'V4_ATTEMPT04_GPU_TEMPERATURE_UNPROVEN'
                ),
    ...
    if set(processes) - {str(device['uuid']) for device in devices}:
        raise V4ExecutionError(
            'V4_ATTEMPT04_GPU_PROCESS_DEVICE_UNPROVEN'
        )
    return {
...
def _device_map(
    ...
    if (
        ...
        or sample.get('base_commit') != BASE_COMMIT
        or sample.get('base_tree') != BASE_TREE
        ...
        raise V4ExecutionError(
            'V4_ATTEMPT04_GPU_INVENTORY_SCHEMA_REQUIRED'
        )
    ...
    for device in devices:
        ...
        if (
            ...
            raise V4ExecutionError(
                'V4_ATTEMPT04_GPU_IDENTITY_UNPROVEN'
            )
        for key in (
            'total_memory_bytes',
            'free_memory_bytes',
...
def _sample_timestamp(value: object) -> float:
    if not isinstance(value, str) or not value.endswith('Z'):
        raise V4ExecutionError(
            'V4_ATTEMPT04_GPU_SAMPLE_INDEPENDENCE_UNPROVEN'
        )
    try:
...
def _candidate_reason(
    ...
    if len(devices) != SAMPLE_COUNT or any(item is None for item in devices):
        return 'V4_ATTEMPT04_GPU_MAPPING_DRIFT'
    ...
    for row in rows:
        ...
        if not isinstance(processes, list):
            return 'V4_ATTEMPT04_GPU_PROCESS_ENUMERATION_UNPROVEN'
        if processes:
            return 'V4_ATTEMPT04_GPU_COMPUTE_PROCESS_PRESENT'
        if row.get('model') != AUTHORIZED_GPU_MODEL:
            return 'V4_ATTEMPT04_GPU_MODEL_NOT_AUTHORIZED'
        if str(row.get('mig_mode')).strip().lower() not in {
            'disabled',
            'off',
            '0',
        }:
            return 'V4_ATTEMPT04_GPU_MIG_ENABLED'
        if str(row.get('compute_mode')).strip().lower() != 'default':
            return 'V4_ATTEMPT04_GPU_COMPUTE_MODE_NOT_DEFAULT'
        if row.get('utilization_gpu_percent') != 0:
            return 'V4_ATTEMPT04_GPU_UNEXPLAINED_ACTIVITY'
        if int(row.get('free_memory_bytes', -1)) < MIN_FREE_MEMORY_BYTES:
            return 'V4_ATTEMPT04_GPU_FREE_MEMORY_INSUFFICIENT'
        if (
            row.get('ecc_health') != 'OK'
            or row.get('ecc_uncorrected_volatile_total') != 0
        ):
            return 'V4_ATTEMPT04_GPU_ECC_HEALTH_ERROR'
    return None


def select_attempt04_gpu(
    samples: Sequence[Mapping[str, object]],
) -> dict[str, object]:
    if len(samples) != SAMPLE_COUNT:
        raise V4ExecutionError(
            'V4_ATTEMPT04_THREE_SAMPLES_REQUIRED'
        )
    maps = [_device_map(sample) for sample in samples]
    if len({str(sample.get('hostname')) for sample in samples}) != 1:
        raise V4ExecutionError('V4_ATTEMPT04_GPU_HOST_DRIFT')
        ...
        evaluations.append(
            {
                ...
                'reason_code': (
                    reason or 'V4_ATTEMPT04_GPU_CANDIDATE_ELIGIBLE'
                ),
                'indices': [
    ...
    return {
        'status': 'PASS',
        'reason_code': 'V4_ATTEMPT04_GPU_SELECTED',
        'candidate_evaluations': evaluations,
...
def _validate_preflight_payload(payload: Mapping[str, Any]) -> None:
    if payload.get('schema_version') != PREFLIGHT_SCHEMA:
        raise V4ExecutionError('V4_ATTEMPT04_PREFLIGHT_SCHEMA_REQUIRED')
    if payload.get('attempt_id') != ATTEMPT_ID:
        raise V4ExecutionError('V4_ATTEMPT04_ID_MISMATCH')
    if (
    ...
    if status == 'PASS':
        if (
            ...
            or payload.get('reason_code') != 'V4_ATTEMPT04_PREFLIGHT_PASS'
        ):
            raise V4ExecutionError('V4_ATTEMPT04_GPU_SELECTION_TAMPER')
    elif status == 'FAIL':
        if (
    ...
    _validate_signature(
        payload,
        'hardware_snapshot_sha256',
        'V4_ATTEMPT04_PREFLIGHT_TAMPER',
    )


def create_attempt04_gpu_preflight(
    *,
    worktree: Path,
    resource_receipt_path: Path,
    output_path: Path,
    ...
    _guard_attempt_phase(output_root, 'preflight')
    ...
    resources = validate_attempt04_resources(resource_receipt_path)
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_TOOLING_MISMATCH')
    sampler = (
...
def validate_attempt04_gpu_preflight(path: Path) -> dict[str, Any]:
    path = _attempt_path(path)
    payload = _load_json(path)
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RESOURCE_RECEIPT_TAMPER')
    resources = validate_attempt04_resources(resource_path)
    runtime_root = Path(str(resources.get('runtime_root')))
    if (
...
def _validate_receipt_payload(payload: Mapping[str, Any]) -> None:
    if payload.get('schema_version') != RECEIPT_SCHEMA:
        raise V4ExecutionError('V4_ATTEMPT04_RECEIPT_SCHEMA_REQUIRED')
    if (
        payload.get('attempt_id') != ATTEMPT_ID
        or payload.get('status') != 'PASS'
        or payload.get('authorization_kind') != 'V4_MVE_EXECUTION'
    ):
        raise V4ExecutionError('V4_ATTEMPT04_RECEIPT_PASS_REQUIRED')
    if (
        not _is_sha(payload.get('run_id'), 32)
        or not _is_sha(payload.get('tooling_commit'), 40)
        or not _is_sha(payload.get('tooling_tree'), 40)
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RECEIPT_TAMPER')
    if payload.get('scientific_anchor_commit') != SCIENTIFIC_ANCHOR_COMMIT:
        raise V4ExecutionError('V4_ATTEMPT04_SCIENTIFIC_ANCHOR_MISMATCH')
    if payload.get('scientific_anchor_tree') != SCIENTIFIC_ANCHOR_TREE:
        raise V4ExecutionError('V4_ATTEMPT04_SCIENTIFIC_ANCHOR_MISMATCH')
    if payload.get('schedule_file_sha256') != SCHEDULE_FILE_SHA256:
        raise V4ExecutionError('V4_ATTEMPT04_SCHEDULE_TAMPER')
    if (
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_DEVICE_SCOPE_MISMATCH')
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_GPU_IDENTITY_UNPROVEN')
    if (
        payload.get('torch_probe_invocations') != 0
        or payload.get('model_loads') != 0
        or payload.get('model_forwards') != 0
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RECEIPT_SCOPE_MISMATCH')
    _validate_signature(
        payload,
        'receipt_payload_sha256',
        'V4_ATTEMPT04_RECEIPT_TAMPER',
    )
...
def _validate_authorization_payload(payload: Mapping[str, Any]) -> None:
    if payload.get('schema_version') != AUTHORIZATION_SCHEMA:
        raise V4ExecutionError(
            'V4_ATTEMPT04_AUTHORIZATION_SCHEMA_REQUIRED'
        )
    if (
        payload.get('attempt_id') != ATTEMPT_ID
        ...
        raise V4ExecutionError(
            'V4_ATTEMPT04_AUTHORIZATION_SCOPE_MISMATCH'
        )
    if (
        not _is_sha(payload.get('run_id'), 32)
        or not _is_sha(payload.get('tooling_commit'), 40)
        ...
        raise V4ExecutionError('V4_ATTEMPT04_AUTHORIZATION_TAMPER')
    ...
    if budget != expected_budget:
        raise V4ExecutionError('V4_ATTEMPT04_BUDGET_SCOPE_MISMATCH')
    if payload.get('finalizer') != AUTHORIZED_FINALIZER:
        raise V4ExecutionError(
            'V4_ATTEMPT04_FINALIZER_SCOPE_MISMATCH'
        )
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RUNTIME_PATH_ESCAPE')
    ...
    _validate_signature(
        ...
        'V4_ATTEMPT04_AUTHORIZATION_TAMPER',
    )
...
def _under_root(root: Path, path: Path) -> Path:
    ...
    try:
        ...
        raise V4ExecutionError('V4_ATTEMPT04_RUNTIME_PATH_ESCAPE') from exc
    return resolved
...
def _atomic_artifact_set(
    ...
    try:
        for (_, payload, validator), partial in zip(
            entries, partials, strict=True
        ):
            partial.write_bytes(_json_bytes(payload))
            validator(_load_json(partial))
        for (path, _, _), partial in zip(entries, partials, strict=True):
            partial.replace(path)
...
def create_attempt04_execution_authorization(
    *,
    worktree: Path,
    runtime_root: Path,
    resource_receipt_path: Path,
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_PATH_SCOPE_MISMATCH')
    _guard_attempt_phase(attempt_root, 'authorization')
    resources = validate_attempt04_resources(resource_receipt_path)
    snapshot = validate_attempt04_gpu_preflight(hardware_snapshot_path)
    if snapshot.get('status') != 'PASS':
        raise V4ExecutionError('V4_ATTEMPT04_PREFLIGHT_PASS_REQUIRED')
    if tooling_commit is None or tooling_tree is None:
        tooling_commit, tooling_tree = _tooling_revision(worktree)
    for evidence in (resources, snapshot):
        if (
            ...
            raise V4ExecutionError(
                'V4_ATTEMPT04_AUTHORIZATION_TOOLING_MISMATCH'
            )
    ...
    if any(Path(value).exists() for value in resolved_paths.values()):
        raise V4ExecutionError('V4_ATTEMPT04_RUNTIME_ARTIFACT_COLLISION')
    receipt = _signed(
        {
    ...
    decision = _decision_payload(
        status='PASS',
        reason_code='V4_MVE_EXECUTION_AUTHORIZED',
        tooling_commit=tooling_commit,
        tooling_tree=tooling_tree,
        resource_receipt_path=resource_receipt_path,
        snapshot_path=hardware_snapshot_path,
        receipt_path=receipt_path,
        receipt_sha256=_sha_json(receipt),
        authorization_path=authorization_path,
        authorization_sha256=_sha_json(authorization),
    )
    generated_paths = (receipt_path, authorization_path, decision_path)
    try:
        _atomic_artifact_set(
            (
                (receipt_path, receipt, _validate_receipt_payload),
                (
                    authorization_path,
                    authorization,
                    _validate_authorization_payload,
                ),
                (decision_path, decision, _validate_decision_payload),
            )
        )
        validate_attempt04_execution_authorization(authorization_path)
    except Exception:
...
def validate_attempt04_execution_authorization(
    path: Path,
) -> dict[str, Any]:
    path = _attempt_path(path)
    payload = _load_json(path)
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_PREFLIGHT_TAMPER')
    snapshot = validate_attempt04_gpu_preflight(snapshot_path)
    resource_path = Path(str(payload.get('resource_receipt_path')))
    if (
    ...
    if (
        ...
        raise V4ExecutionError('V4_ATTEMPT04_GPU_IDENTITY_TAMPER')
    ...
    if (
        ...
        or decision.get('gpu_receipt_path') != str(receipt_path.resolve())
        or decision.get('gpu_receipt_sha256') != _sha256_file(receipt_path)
        or decision.get('execution_authorization_path')
        != str(path.resolve())
        or decision.get('execution_authorization_sha256')
        != _sha256_file(path)
        or decision.get('hardware_snapshot_sha256')
        != _sha256_file(snapshot_path)
        or decision.get('resource_receipt_sha256')
