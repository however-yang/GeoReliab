...
def build_ledger_chain_bridge(
    *,
    ...
    The bridge is metadata only: it never truncates or rewrites the source
    ledger and never interprets opaque torn bytes as events. A future journal
    may append at ``next_sequence_index`` with ``previous_event_sha256`` and
    must retain this bridge digest alongside the new segment.
    """
